    ...
    <el-container style="height: 100%;">
      <el-aside width="22%" class="no-horizontal-scroll">
        ...
        <el-row>
          ...
          <el-col :span="12">
            得分：{{ paperInfo.finalScore }} 分
          </el-col>
          <el-col :span="12">
            耗时：{{ paperInfo.timeConsuming }}
          </el-col>
        </el-row>
          ...
          <el-col>
            <el-row>
              <el-col :span="4" v-for="(question, index) in questionIndexList" :key="index">
                <el-button
                  :style="{ backgroundColor: answeredQuestions.includes(question.titleIndex + '-' + question.questionIndex) ? '#66c13a' : '#f56d6d' }"
        ...
        <el-row>
          <el-col :span="24" class="text-center">
            <el-button type="primary" @click="submitPaper">提交批改</el-button>
          </el-col>
        </el-row>
      </el-aside>
            ...
            <el-col :span="24" v-for="(column, titleIndex) in paperInfo.paperTopicList" :key="titleIndex">
              ...
              <div class="question-item"
                ...
                <span v-if="question.questionsType == 1">
                  <el-radio-group v-model="question.userCorrectOptionKey" class="radio-group" @input="changeRadio(titleIndex,paperQuestionsIndex)">
                    <el-radio disabled style="margin-top: 10px;" v-for="(bankColumn, bankIndex) in question.bankOptionsList"
                  ...
                  <el-descriptions style="margin-top: 20px" :column="1">
                    <el-descriptions-item label="答案"><span v-html="question.correctOptionKey"></span></el-descriptions-item>
                    <el-descriptions-item label="结果">
                      <el-tag type="success" v-if="question.status==1">正确</el-tag>
                      <el-tag type="danger" v-else>错误</el-tag>
                    </el-descriptions-item>
                    <el-descriptions-item label="得分">{{question.status==1?question.score:0 + '/' + question.score}}</el-descriptions-item>
                    ...
                    <el-descriptions-item label="解析">
                      <div class="analysis-content" v-html="question.questionsAnalyze"></div>
                    </el-descriptions-item>
                  </el-descriptions>
                </span>
                <span v-if="question.questionsType == 2">
                   <el-checkbox-group disabled v-model="question.userCorrectOptionKey" @change="changeCheckbox(titleIndex,paperQuestionsIndex)">
                ...
                <span v-if="question.questionsType == 3">
                   <el-radio-group disabled v-model="question.userCorrectOptionKey" class="radio-group" @input="changeRadio(titleIndex,paperQuestionsIndex)">
                ...
                <span v-if="question.questionsType == 4">
                  <div>
                    <el-row>
                      <el-col :span="24">
                        <span style="float: left;margin-left: 10px;margin-top: -14px;background-color: #f3f3f3" v-html="question.userCorrectOptionKey"></span>
                      </el-col>
                    </el-row>
                     <el-row>
                      <el-col :span="24">
                    ...
                    <el-descriptions style="margin-top: 20px" :column="1" >
                      <el-descriptions-item label="答案" class="analysis-content"><span class="analysis-content" v-html="question.correctOptionKey"></span></el-descriptions-item>
                      <el-descriptions-item label="结果">
                        <el-tag type="success" v-if="question.status==1">正确</el-tag>
                        <el-tag type="danger" v-else>错误</el-tag>
                      </el-descriptions-item>
                      ...
                      <el-descriptions-item label="批改">
                        <el-input-number v-model="question.markScore" :precision="2" :step="1.00" :max="question.score" :min="0"></el-input-number>
                      </el-descriptions-item>
                    </el-descriptions>
                  </div>
                </span>
                <span v-if="question.questionsType == 5">
                  <div v-for="(bankColumn, bankIndex) in question.bankOptionsList" :key="bankIndex" class="option-item"
                    ...
                    <el-row>
                      <el-col :span="6">
                        <span class="answer" style="margin-left: 0px; display: inline-block;float: right" v-html="bankColumn.userAnswer"></span>
                      </el-col>
                      <el-col :span="12">
                          <span class="answer" style="margin-left: 40px; display: inline-block;float: right;color: red" v-html="bankColumn.answer"></span>
                      </el-col>
                    </el-row>
                  </div>
                  <el-descriptions style="margin-top: 20px" :column="1">
...
import { addPaperRecord, editPaperRecord, getRecordById } from '@/api/education/record'
    ...
    dataConversion() {
      ...
      for (let i = 0; i < this.paperInfo.paperTopicList.length; i++) {
        for (let j = 0; j < this.paperInfo.paperTopicList[i].paperQuestionsList.length; j++) {
          ...
          let question = this.paperInfo.paperTopicList[i].paperQuestionsList[i];
          if (question.status==1) {
            this.updateAnsweredStatus(i,j);
          }
        }
      }
    },
    submitPaper() {
      this.paperInfo.paperTopicList.forEach(topic => {
        topic.paperQuestionsList.forEach(question => {
          if (question.questionsType ==2 && question.userCorrectOptionKey != undefined && question.userCorrectOptionKey != '') {
            question.userCorrectOptionKey = question.userCorrectOptionKey.join(",");
          }
        })
      })
      this.$confirm('提交批改?', '提示', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning'
      }).then(() => {
        editPaperRecord(this.paperInfo).then(response => {
